...
## <a id="rdp_workflow"></a>RDP APIs Authentication
...
Next, import this ```RDP_AuthToken_Type``` Type to the main application ```rdp_nodefetch.ts``` file. Please note that the RDP APIs endpoint and credentials information will be passed to Node.js runtime via Docker environment variables. 
...
Then we create a function ```authenRDP``` to send a login request message to the RDP Auth Token service. The function sends HTTP Post messages to the RDP via Node native Fetch API. 
...
Once the authentication success, the function gets the RDP Auth service response message (via *server.js*), and keeps the following RDP token information in the variables.
...
Now the example application supports both the Password Grant and Refresh Grant scenarios for the RDP APIs with a single ```authenRDP()``` function. We can put this function call inside a ```main()``` function.
...
This project demonstrates the following the RDP Services:
...
This example get convert a symbol from the RIC Code identifier to [ISIN](https://en.wikipedia.org/wiki/International_Securities_Identification_Number), [LEI](https://en.wikipedia.org/wiki/Legal_Entity_Identifier), and [ExchangeTicker](https://en.wikipedia.org/wiki/Ticker_symbol) identifiers.  The first step is creating Type Aliases for the Symbology HTTP Post message.
...
Next, create a function ```requestSymbol``` to create the Discovery Symbology JSON request message in the main  ```rdp_nodefetch.ts``` file. This ```requestSymbol()``` just sends the HTTP Post request message to RDP Service and returns the response data in JSON message format.
...
main()
```
That is all for the getting Symbology data part. The next step is displaying incoming Symbology data in a readable format. The code uses the [console.table()](https://nodejs.org/docs/latest-v18.x/api/console.html#consoletabletabulardata-properties) function to print data to a console in a tabular format.

Let's start by creating a new Type Aliases for the Symbology table object named ```symbologyTable```. This object keeps the necessary output data which are ```identifierType```, ```value```, ```name```, and ```status``` fields from the JSON message.
...
Finally, we create a ```displaySymbology()``` to construct the ```symbologyTable``` object and then passes it to the ```console.table()``` function.
